...
import reducer from './burguerConstructor';
import sinon from 'sinon';
import { Ingredients } from '../system/index';
...
describe('burguerConstructor reducer', () => {
    ...
    const setIngredientsMock= sinon.mock({
        salad: 1,
		bacon: 1,
		cheese: 1,
		meat: 1
	...
	describe('Add Ingredients', () => {
		...
		it('should return initial state if ingredients is null', () => {
			expect(reducer(initialState, { type: actionTypes.ADD_INGREDIENT, ingredients: 'meat' })).toEqual({
				ingredients: null,
				totalPrice: 4,
				error: false,
				loading: true
    ...
    describe('Remove Ingredients', () => {
		...
		it('should return initial state if ingredients is null', () => {
			expect(reducer(initialState, { type: actionTypes.REMOVE_INGREDIENT, ingredients: 'meat' })).toEqual({
				ingredients: null,
				totalPrice: 4,
				error: false,
				loading: true
